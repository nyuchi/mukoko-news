...
>
> **At 11.06 s the directory read now clears `QUERY_MAX_TIME_MS` (15 s) and completes rather than aborting.** But ⚠️ **the binding constraint has moved to the Vercel function.** There is no `maxDuration` anywhere in this repo and no `vercel.json`, so the platform default applies — **10 s on Hobby, 15 s on Pro** — and an 11 s query does not fit a 10 s function at all, nor a 15 s one once connection setup, the variant fold, the profile read and render are added. On a cold `unstable_cache` the request will very likely be killed by Vercel before Mongo's own bound ever fires. Three ways out, none of them chosen here: `export const maxDuration` on the author route (one line, unblocks, makes nothing fast); moving the directory build off the request path entirely so no reader waits on it (the real fix); or resizing the instance — 11 s for 58,810 **covered** index keys is ~8,250 keys/sec, which is far slower than that work should take and is unlikely to be this query's problem alone.
>
