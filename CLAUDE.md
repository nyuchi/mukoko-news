...
**Deferring to the system — but only where the system asked to take over** (corrected 2026-09-11). `color-scheme` is declared per theme so native controls, scrollbars and the first-paint canvas match. The two contrast queries are then **not** treated alike:
