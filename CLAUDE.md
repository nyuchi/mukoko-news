...
>
> ⚠️ **It is necessary but NOT sufficient, and the remainder is not a query problem.** The same covered query measured **16.7 s, then 19.0 s, then 46.6 s** on three consecutive runs — identical plan, identical 58,660 keys, identical zero documents. A 3× spread on identical work is capacity, not shape: the cluster is an **M20** (2 burstable vCPU, 4 GB RAM, AWS af-south-1) hosting a 1.5 GB hot collection, 14 classic indexes, 3 Atlas Search indexes and the platform's other ~29 databases, and those measurements exhausted its CPU credits as they ran. Removing the `$ne` pair was tested and made it **slower** (19.0 s), so the `$ne` clauses are not the cost either. **Byline pages will still exceed the 15 s bound until the cluster has the headroom to scan 58,660 index keys quickly** — re-measure on recovered credits before drawing any further conclusion, and treat instance sizing as the open question rather than the query.
>
