...
>
> `avgQualityScore` is now `number | null` and is **null**: there is no index on `qualityScore` and no Atlas Search mapping for it, so averaging it means reading every document. `0` would render as *"this corpus scores zero on quality"*. ⚠️ **Still to do**: `getPublishingVolume`, `getSourceLeaderboard`, `getCategoryDistribution`, `getSentimentBreakdown` and `getTopTopics` remain document-scanning and will therefore degrade to their empty states on this cluster. Volume, the leaderboard and the category split can move to `$searchMeta` (a date-boundary facet over `datePublished` and string facets over `feedSourceId` / `categoryIds` are all mapped, and were verified working against the live index); sentiment and quality cannot, and need either those fields added to the Atlas Search index or a rollup written by the pipeline.
...
>
> **Components separate by FILL, not by a drawn edge** (owner decision 2026-09-10). Every card, panel and chip carried `border border-border`, so the product looked like a high-contrast theme nobody chose — and with everything boxed, nothing read as emphasised. There is now one token: **`--outline`, transparent by default**, and every boxed component draws `border border-outline`. **Switched on it takes `--raised`, not `--border`** (corrected 2026-09-11 — owner report: *"the contrast component doesn't work, I see no visual changes"*). `--border` is `#2A2927` in dark and `#E7E5E0` in light against a card of `#131211` / `#EEEEEC` — a step of roughly 2% in lightness, real in a colour picker and invisible on a phone, so the control did exactly what it claimed and the reader could not tell, which is indistinguishable from a control that does nothing. `--raised` (`#2E2C29` / `#D6D5D1`) is the next step the scale actually provides, plainly visible in both themes and still on palette; `--border` keeps its own job, separators, where the line divides two things already adjacent and needs to be quiet. The border stays in the box model, so switching it on shifts no layout. `--border` keeps its colour and stays visible for **separators** (`border-b`, `border-t`, `divide-y`, the byline rule) — a line meaning "these two things are different" is a different job from a line meaning "here is an edge", which is why they are now different tokens. The reader picks one of **three** values on **/profile → Appearance**, applied pre-paint by the bootstrap script in `layout.tsx`: the attribute **absent** (or `off`) is no outlines and is the default, `data-outlines="on"` is always outlined, and `data-outlines="system"` is outlined only under **`prefers-contrast: more`**. Every rule that draws an edge names `on` or `system` **positively** — nothing keys off the attribute's absence — so a reader whose bootstrap never ran (JS off, blocked storage, a throw) lands on the quiet look rather than on an outlined app they cannot switch off. ⚠️ **Corrected 2026-09-11 (owner report), superseding "not optional under `prefers-contrast: more`".** That rule turned outlines on regardless of the setting, on the argument that a reader who asked their OS for more contrast is asking for the edge. Measured on the owner's phone with the OS *Increase Contrast* switch on, it produced every card, panel, chip and the nav pill outlined while the Appearance card showed **"Off — Separated by fill" selected** — *"the contrast is still appearing in dark mode even though it's off"*. A setting that says Off and is not off is worse than no setting, so the OS query became the third **choice** rather than an override on top of the other two. The OS signal is not discarded: `prefers-contrast: more` still lifts `--text-secondary`/`--text-tertiary` to full foreground **unconditionally**, because that half of *"make differences easier to see"* is about reading a sentence rather than about drawing boxes; only the edge tokens (`--outline`, `--border`, `--control`) are the reader's call. `design-tokens.test.ts` asserts structurally that **every** declaration of an edge token inside that block is qualified by a `[data-outlines=…]` selector. (Outlines are **not** on there because the surfaces stop separating: that block keeps every Mzizi step. An earlier version replaced them all with `Canvas`, and this sentence used to cite that as the reason — see the corrected note under **Deferring to the system**.) A **control's** edge is a different job again and gets its own token, **`--control`** (always visible, aliased to `--border` so it follows into the high-contrast block): an `<input>` or `<select>` painted with the transparent `--outline` would be an undiscoverable field, and a button's `outline` variant IS its edge. There are deliberately **no file-level exemptions** in the check — exempting a file would let a future card in that same file slip through, which is how the borders got everywhere in the first place. `src/lib/__tests__/appearance.test.ts` structurally forbids a box outline drawn on `--border`/`--elevated` anywhere else. ⚠️ This **diverges from the Mzizi card spec**, which specifies "a full 1px border" (`mzizi_get_tokens: componentSpecs`) — reported upstream rather than silently forked.
>
...
- **`@media (prefers-contrast: more)` raises contrast INSIDE the Mzizi scale.** Every surface keeps its step; `--text-secondary`/`--text-tertiary` lift to full foreground **unconditionally**; and — for a reader on `data-outlines="on"` or `"system"` only — outlines switch on and `--border`/`--control` take the value `--text-tertiary` normally holds (`#474139` / `#D4D4CF`), a measured, on-palette edge. A reader who chose (or defaulted to) **off** gets the text lift and nothing else; see the outline note above. ⚠️ This **supersedes** the earlier rule that handed every reading surface to `Canvas`/`CanvasText`. Measured on a phone with the OS "Increase Contrast" switch on, that rule collapsed all **eight** background steps into one flat system colour and outlined every card, chip and the nav pill in stark white on black: the card stopped reading as a card, because the fill separating it from the page was gone and the border was the only structure left. `prefers-contrast: more` means *make differences easier to see*, not *throw away the palette* — the Mzizi scale is not low contrast, it is eight deliberately distinct steps, and it is the thing carrying the structure.
...
**Contrast is part of the THEME** (owner direction 2026-09-11 — *"the contrast switcher is part of the theme"*). `ThemeProvider` (`components/theme-provider.tsx`) owns it: `useTheme()` returns `{ theme, resolvedTheme, setTheme, cycleTheme, contrast, setContrast }`, and the provider is the only runtime writer of the storage key and the `data-outlines` attribute. It used to be a separate preference that the one card rendering it read, stored and applied itself, so nothing else in the app could ask what it was set to or react to it. `src/lib/appearance.ts` stays as the pure parse/store/apply module, because the pre-paint bootstrap in `layout.tsx` repeats its key and attribute as string literals and cannot import anything — a test asserts the two still agree.

**Appearance is picked by looking** (`components/profile/profile-appearance.tsx`). Theme *and* outlines are one card on `/profile`, **three options each**, and every option renders a **miniature of the app** in that setting — a header bar, a card holding two text lines, and the page behind both. The outline row's **System** draws both halves of the same theme, plain beside outlined, because whether it draws an edge depends on a `prefers-contrast` query the component cannot read and committing to either picture would advertise the wrong one to half the readers who see it (`AppearancePreview`'s `outlined="split"`, one axis over from the theme row's light/dark split). Before this, outlines were a lone switch with a paragraph explaining it and the theme was a **cycle button** elsewhere on the page showing only its current value, so a reader tapped three times to discover the options and never saw two side by side. Both are purely visual settings; the honest control is the thing itself. `appearance-preview.tsx` is the **one place a literal hex is correct** — it must draw a theme the document is *not* in, and a `bg-surface` would paint all three options in the active theme — so its values are asserted against `globals.css` by test rather than trusted.
