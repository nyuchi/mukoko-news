...
- **The app never touches the pipeline** (owner rule 2026-09-25). The pipeline sits behind the databases; the gateway Worker is the app's backend. The manual feed refresh (`src/lib/actions/refresh.ts`) calls the gateway's `POST /api/refresh`, which holds the pipeline credential and applies a per-caller cooldown. The app holds no `FLY_*` secret.
...
>
> ⚠️ **That unblocked; it made nothing fast.** The real fix followed, below. Resizing the instance is still open — 11 s for 58,810 **covered** index keys is ~8,250 keys/sec, far slower than that work should take and unlikely to be this query's problem alone.

### The directory is a SNAPSHOT now, built by a cron (owner decision 2026-09-19)

**`getBylineDirectory` no longer runs on a reader's request.** It was behind `unstable_cache` for an hour, which does not remove an 11-second query — it picks **one unlucky reader per hour per region** to pay for it, and that reader is usually a crawler on a byline page nobody has opened recently, i.e. the request you least want to spend 11 s on. A TTL is not a fix for a slow build; it is a fix for a _frequent_ one.

|                  | before                                                                | after                                         |
| ---------------- | --------------------------------------------------------------------- | --------------------------------------------- |
| resolving a slug | `$group` over the corpus, ~58,700 keys, **11,057 ms** on a cold cache | one `_id` point lookup, **~1 ms**             |
| who pays         | a reader                                                              | nobody — `/api/cron/byline-directory`, hourly |
| cache            | `unstable_cache`, 1 h, per region                                     | none; the snapshot IS the cache               |

`news.bylineDirectory` holds one document per slug (`_id` **is** the slug, so the read is covered by the index every collection already has). `src/lib/mongodb/byline-directory.ts` owns both ends: `lookupBylineIdentity` for the read, `publishBylineDirectory` for the build.

**Publishing is upsert-then-sweep, never wipe-then-fill.** Every row is stamped with the build's `generation`, all rows are upserted, and only then is everything **older** deleted. Until the sweep runs a reader sees the previous snapshot; after it, the new one. There is **no window in which the collection is empty** — which matters, because an empty collection is indistinguishable from a platform with no journalists.

⚠️ **The sweep filter is `{generation: {$lt: …}}`, and `$ne` there is a platform-wide outage.** `generation` is an ISO-8601 timestamp, so lexicographic order is chronological and a build sweeps only what is older than itself. Under `$ne`, two overlapping builds destroy the directory: `A` upserts all, `B` upserts all, then `A` sweeps everything that is not `A` — which by then is every row `B` wrote. The build takes ~11 s, so a manual seed overlapping the hourly cron is enough to hit it, and the result is every author page answering 5xx until the next build. `byline-directory.test.ts` reads the filter off the real `deleteMany` call and evaluates it against a row a concurrent build would have left, so the assertion is about the code rather than about a filter the test wrote itself.

**Seed the snapshot BEFORE the change deploys, not after.** `scripts/seed-byline-directory.ts` runs the same `getBylineDirectory` → `publishBylineDirectory` pair the cron route runs, from a terminal, against `MONGODB_URI` — so it depends on nothing from the deployed build (`MONGODB_URI=… pnpm dlx tsx scripts/seed-byline-directory.ts`; `dlx` rather than a devDependency, so `pnpm-lock.yaml` does not grow a runner no build uses). Deploying first and waiting for the first cron means every byline page 5xxs in the meantime — the honest answer for an unbuilt directory, but an avoidable outage. There is deliberately **no second implementation of the write** in the script: a separate definition of the snapshot's shape is free to drift from the one the app reads, and the first symptom of that drift is a real journalist's page 404ing. A test asserts the script imports the shared pair and never touches the collection itself.

⚠️ **An empty build is REFUSED over a populated snapshot**, and a build whose source read failed is refused outright. Both arrive as `[]` and only one of them is a corpus; publishing the wrong one would 404 every journalist on the platform until the next cron. The cron answers **503** (source unavailable) or **409** (empty build) rather than a cheerful 200, so a cron that has quietly stopped producing a directory is visible in the log instead of looking like a successful no-op.

⚠️ **An unbuilt snapshot is `unavailable`, never `not-found`** — this is the same trap as the original outage, wearing new clothes. A fresh environment, a dropped collection or a cron that has never fired all look exactly like "no such byline". So a miss costs one extra `estimatedDocumentCount` to find out _which kind_ of miss it is: rows present → `not-found` (we looked, and we looked in something); zero rows → `unavailable`. `author-directory-availability.test.ts` drives the real driver through the real reader and the real action and asserts it, plus the load-bearing negative: **no page render may issue the `$author.name` scan under any circumstance.** Verified non-vacuous — adding a "fall back to a live build when the snapshot is missing" fails three tests by name.

**The cron is authenticated by `CRON_SECRET` and by nothing else.** The `vercel-cron/1.0` UA is not a check — it is spoofable — and an unset secret answers **503**, never "allow". An open rebuild endpoint is an 11-second cluster query any stranger can fire in a loop, which points a denial-of-service knob at the whole platform rather than at `/author`. Comparison is `timingSafeEqual` with a length pre-check.

⚠️ **`vercel.json` schedules it hourly (`0 * * * *`), matching the TTL it replaces.** Vercel's Hobby plan restricts cron frequency; if the project is on Hobby this may run only once a day (a stale directory, not a broken one — a new byline's page 404s until the next build) or the deploy may reject the schedule, in which case it is a one-line change to `0 4 * * *`. The snapshot must also be **seeded once** after the first deploy: until it exists, author pages answer 5xx, which is the honest answer and deliberately not a 404.
...
- **Feed refresh** — `src/lib/actions/refresh.ts` `triggerFeedCollection()` fire-and-forget `POST`s to the gateway's `/api/refresh`. It used to call the Fly worker's `/trigger/collect` directly with `FLY_TRIGGER_TOKEN`; that is the pipeline touching the app, and it is gone.
...
# Gateway Worker (the app backend: admin mutations + feed refresh)
