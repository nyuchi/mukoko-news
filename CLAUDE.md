...
> ⚠️ **Measured 2026-09-17 — why every byline page said "Byline not found" over real journalists.** This paragraph used to claim that both reads "are windowed to 365 days so they ride `status_1_datePublished_-1` as a range seek". **They do not, and that sentence is why nobody looked.** Two independent reasons, and only one of them is about the index: (1) `VISIBLE` opens with `status: {$ne: 'rejected'}`, and a `$ne` on a compound index's LEADING key is a range over the whole key space less one point — with a range rather than an equality there, the `datePublished` bound on the second key cannot narrow the scan's entry point. This is the identical shape, on the identical collection, as the `/insights` incident above: 27,529 ms, 65,203 documents examined, **zero index keys**. (2) The window is **wider than the corpus** — the oldest articles are from 2026-05 — so a 365-day bound excludes nothing and even a perfect seek returns every key. `author.name`, `moderationStatus` and `mediaOrganizationId` are in no index at all (classic or Atlas Search), so every candidate is FETCHed from the 1.5 GB collection regardless. `getBylineDirectory`'s `$group` did not return within 60 s on a direct connection; `QUERY_MAX_TIME_MS` (15 s) aborts it on every render.
>
> **The required cluster change** — not made from this repo, which creates no indexes and ships no migrations — is `{'author.name': 1, datePublished: -1}` on `news.articles`. It is decisive for `getAuthorProfile`, whose `$in` on one to three exact spellings becomes a handful of seeks. `getBylineDirectory` has no equality to seek on, so it additionally wants the read COVERED — `{'author.name': 1, datePublished: -1, mediaOrganizationId: 1, status: 1, moderationStatus: 1}` — or its `author.name` mapped into `articles_insights` so a `$searchMeta` facet can answer it the way the Insights panels are answered. **Nothing in this repo can fix the timeout.**
>
