...
async def refresh_trending() -> None:
    ...
    async with pool.acquire() as conn:
        async with conn.transaction():
            ...
            await conn.execute(
                "DELETE FROM trending_cache WHERE scope = 'global'"
            )
        ...
        countries = await conn.fetch(
            "SELECT DISTINCT id FROM countries WHERE enabled = TRUE"
        )
        ...
        for country in countries:
            ...
            if country_rows:
                async with conn.transaction():
                    await conn.execute(
                        "DELETE FROM trending_cache WHERE scope = 'country' AND scope_id = $1",
                        country_id,
                    )
